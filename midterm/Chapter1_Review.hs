...
-- | =================================================================
-- | 7. 高階函數應用範例 (Higher-Order Function Examples)
-- | =================================================================

-- | `forktimes` 接收兩個函數 `f` 和 `g`，以及一個值 `x`。
-- | 它會將 `x` 同時傳給 `f` 和 `g`，然後將兩個結果相乘。
-- | 這是高階函數的一個簡單應用，將「如何組合結果」(`*`) 與「如何產生結果」(`f`, `g`) 分開。
forktimes :: (a -> Int) -> (a -> Int) -> a -> Int
forktimes f g x = f x * g x

-- | **問題：** 計算 `(x+1) * (x+2)`
-- | **思路：** 這次我們用 `forktimes` 來組合 `(+1)` 和 `(+2)` 兩個函數。
polyCompute_v1 :: Int -> Int
polyCompute_v1 = forktimes (+1) (+2)

-- | =================================================================
-- | 8. 綜合應用 (lift2)
-- | =================================================================

-- | `lift2` 是 `forktimes` 的一個更泛用、更抽象的版本。
-- | 它不限定最終必須用 `(*)` 來組合，而是可以接受任何二元函數 `h`。
-- | `forktimes f g x` 事實上等價於 `lift2 (*) f g x`。
